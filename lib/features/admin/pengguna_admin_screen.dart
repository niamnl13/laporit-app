...
class _PenggunaAdminScreenState extends State<PenggunaAdminScreen> {
  ...
  final List<String> _roleFilters = ['Semua User', 'Operator', 'Admin'];
  ...
  Future<void> _toggleAktif(dynamic user) async {
    final id = user['id'];
    final isAktif = user['is_active'] == true || user['is_active'] == 1;
    try {
  ...
  Widget _buildUserCard(dynamic user) {
    ...
    final isAktif = user['is_active'] == true || user['is_active'] == 1;
    final roleColor = role == 'admin' ? AppColors.error : AppColors.accent;
    final roleLabel = role == 'admin' ? 'ADMIN' : 'OPERATOR';
